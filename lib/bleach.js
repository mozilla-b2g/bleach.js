...
try {
  ...
  var sanitizeOptions = {
    ignoreComment: opts.stripComments || DEFAULTS.stripComments,
    allowedStyles: opts.styles || DEFAULTS.styles,
    allowedTags: opts.tags || DEFAULTS.tags,
    stripTag: opts.strip || DEFAULTS.strip,
...
HTMLSanitizer.prototype = {
  start: function(tag, attrs, unary) {
    if (this.options.allowedTags.indexOf(tag) === -1) {
    ...
    var whitelist = this.options.allowedAttributesByTag[tag];
    var wildAttrs = this.options.wildAttributes;
    var result = "<" + tag;
  ...
  chars: function(text) {
    if (this.isInsideStyleTag) {
      this.output += CSSParser.parseBody(text, this.options.allowedStyles);
      return;
    }
...
var CSSParser = {
  ...
  parseBody: function (data, allowedStyles) {
    ...
    for (var i = 0; i < rules.length; i++) {
      ...
      var key = rule[0].trim();
      var value = this.parseAttribute(rule[1].trim(), allowedStyles);
      if (value)
        body += key + ' {' + value + '}';
    }
